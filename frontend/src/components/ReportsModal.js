...
const ReportsModal = (props) => {
  ...
  console.log("Show reports for", modalState);
  ...
  useEffect(() => {
    // Your effect logic
    const reportsList = [modalState.file_address].map((report) => ({
      url: "http://localhost:8000/" + report,
      extension: modalState.file_type,
    }));
    console.log("rlist", reportsList);
    setImageReports(
      reportsList.filter(
        (report) =>
          report.extension === "png" ||
          report.extension === "jpg" ||
          report.extension === "jpeg"
      )
    );
    setPdfReports(reportsList.filter((report) => report.extension === "pdf"));
  }, [modalState]);
  ...
  return (
    <Container>
      <ModalContainer>
        ...
        <ModalSubHeading>Reports :</ModalSubHeading>
        ...
        <ModalFooter>
          ...
          {medicalHistory && (
            <Button onClick={() => setReportsModalState(false)}>Close</Button>
          )}
